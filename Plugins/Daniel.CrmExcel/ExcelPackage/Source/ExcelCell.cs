...
namespace OfficeOpenXml
{
	...
	public class ExcelCell
	{
        ...
        #region ExcelCell DateValue
        public DateTime DateValue
	    {
	        get
	        {
	            return DateTime.FromOADate(Convert.ToDouble(this.Value));
	        }
            set
            {
                this.Value = DMYToExcelSerialDate(value.Day, value.Month, value.Year).ToString();
            }
    	}

        private int DMYToExcelSerialDate(int nDay, int nMonth, int nYear)
        {
            try
            {
	    ...
	    public string StyleFormat(ExcelWorkbook workbook)
	    {
	        //XDocument xdoc = XDocument.Load("c:\\aaaaa\\xl\\styles.xml");
	        XDocument xdoc = XDocument.Parse(workbook.StylesXml.InnerXml);
            XNamespace xNamespace=  @"http://schemas.openxmlformats.org/spreadsheetml/2006/main";
            //.Descendants("styleSheet")
            var resultEntity = from c in xdoc.Descendants(xNamespace + "numFmt")
                               select c.Attribute("formatCode");
	        XAttribute[] results = resultEntity.ToArray();
            return results[this.StyleID-1].Value;
	    }
		#endregion
        ...
        private static string GetXpathString(string input)
        {
            ...
            string[] fragments = input.Split(new char[] {Convert.ToChar("'")});
            string result="";
            result+="concat(";
		...
		private int SetSharedString(string Value)
		{
            ...
            XmlNode stringNode = _xlWorksheet.xlPackage.Workbook.SharedStringsXml.SelectSingleNode(string.Format("//d:si[d:t={0}]", GetXpathString(Value)), _xlWorksheet.NameSpaceManager);
			if (stringNode == null)
			{
