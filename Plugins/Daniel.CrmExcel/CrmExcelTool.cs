...
namespace Daniel.CrmExcel
{
        ...
        //IGitHubPlugin IPayPalPlugin
    {
        ...
        private void cmdGenerateExcelSheet_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.Default;
                var logger = new XrmToolBox.Extensibility.LogManager(typeof(CrmExcelTool), ConnectionDetail);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            foreach (TreeNode node in tvwEntities.Nodes["ROOT"].Nodes)
            {
                if (node.Text.ToLower().Contains("account") ||
                    node.Text.ToLower().Contains("contact") ||
                    node.Text.ToLower().Contains("systemuser") ||
                    node.Text.ToLower().Contains("appointment") ||
                    node.Text.ToLower().Contains("letter") ||
                    node.Text.ToLower().Contains("task") ||
                    node.Text.ToLower().Contains("phonecall") ||
                    node.Text.ToLower().Contains("activitypointer") ||
                    node.Text.ToLower().Contains("activityparty") ||
